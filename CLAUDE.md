...
## 세션 시작 시 필수 확인 (Claude 전용)
새 세션을 시작할 때, 다음 순서로 컨텍스트를 로드:
1. **`.claude/context/MEMORY.md`** 를 읽어 인덱스 파악
2. 인덱스에 나열된 각 파일을 읽어 프로젝트 상태(진행 중인 일정, 결정 사항, 행동 규칙)를 회복
3. 이후 작업은 그 컨텍스트를 반영해 진행

이 컨텍스트는 git으로 동기화되므로, 다른 컴퓨터에서 동일 프로젝트를 열어도 같은 상태를 회복할 수 있음. 자세한 운영 방식은 `.claude/context/README.md` 참고.
